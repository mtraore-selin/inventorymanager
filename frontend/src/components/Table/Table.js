    ...
    <div className="dataTable">
          ...
          overlay={
            <Popover
              ...
              style={{
                ...
                boxShadow: "rgb(0 0 0 / 75%) 0px 0px 16px -5px",
              }}
            >
              <Popover.Body>
                <div className="px-2">
                          ...
                          {!unsetColumns.includes(val) && (
                            ...
                            >
                              <svg
                                width={15}
                                height={15 / 1.06}
                                viewBox="0 0 1201.000000 1131.000000"
                              >
                                <g
                                  transform="translate(0.000000,1131.000000) scale(0.100000,-0.100000)"
                                  fill="#2cd498"
                                  stroke="none"
                                >
      ...
      <table className="mytable">
        <thead>
          <tr>
            {props.headers
              .filter((x) => !unsetColumns.includes(x))
              .map((col, i) => {
                return (
                  <th key={i} style={{ width: props.custom_styles[i] }}>
                    <div className="d-flex justify-content-center">
                      ...
                      {props.sortColumn !== "" &&
                        props.columnOriginalNames.findIndex(
                          (x) => x === props.sortColumn
                        ) +
                          1 ===
                          i && (
                          <>
        ...
        <tbody>
          {props.data.length === 0 ? (
            ...
            props.data.map((obj, i) => {
              let fobj = { ...obj };
              let arr = unsetColumns.map((a) =>
                props.headers.findIndex((b) => b === a)
              );

              arr
                .map((x) => Object.keys(fobj)[x])
                .forEach((x) => {
                  delete fobj[x];
                });
              ...
              return (
                <tr key={i}>
                  {Object.keys(fobj).map((key, j) => {
                    return (
                      <td key={j} style={{ width: props.custom_styles[j] }}>
                        {key === "image" ? (
                          <img
                            src={fobj[key]}
                            alt="Image"
            ...
            <td colSpan={props.headers.length}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <button
                  ...
                  onClick={() => {
                    props.tablePageChangeFunc(props.current_page - 1);
                  }}
                  style={{
                    paddingTop: "0.6rem",
