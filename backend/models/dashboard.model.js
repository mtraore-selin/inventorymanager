const { WRONG_MSG } = require("../constants.js");
...
class Dashboard {
  ...
  getProductStats = (req, res) => {
    try {
      new Promise((resolve, reject) => {
        ...
        let p2 = new Promise((rs, rj) => {
          let q =
            "SELECT name, product_stock FROM products ORDER BY product_stock LIMIT 5";
          db.query(q, (err, result) => {
            if (err) {
              rj(err);
            }
  ...
  getGraphStats = (req, res) => {
    try {
      new Promise((resolve, reject) => {
        let q1 =
          "SELECT SUM(grand_total) as Total, DATE_FORMAT(timeStamp, '%Y-%m') as YearMonth FROM `orders` WHERE DATE_FORMAT(timeStamp, '%Y-%m') > DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 12 MONTH),'%Y-%m') GROUP BY DATE_FORMAT(timeStamp, '%Y-%m') ORDER BY timeStamp";
        db.query(q1, (err1, result1) => {
          if (err1) {
            reject(err1);
          }
          let q2 =
            "SELECT SUM(grand_total) as Total, DATE_FORMAT(timeStamp, '%Y-%m') as YearMonth FROM `expenses` WHERE DATE_FORMAT(timeStamp, '%Y-%m') > DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 12 MONTH),'%Y-%m') GROUP BY DATE_FORMAT(timeStamp, '%Y-%m') ORDER BY timeStamp";
          db.query(q2, (err2, result2) => {
            ...
            allYearMonths.forEach((yearMonth) => {
              const order = orders.find(
                (order) => order.YearMonth === yearMonth
              ) || { Total: 0 };
              const expense = expenses.find(
                (expense) => expense.YearMonth === yearMonth
              ) || { Total: 0 };

              res.push({
                YearMonth: yearMonth,
                Month: new Date(yearMonth + "-01").toLocaleString("default", {
                  month: "long",
                }),
                order: order.Total,
