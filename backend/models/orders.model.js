...
class Order {
  getOrders = (req, res) => {
    try {
      ...
      new Promise((resolve, reject) => {
        ...
        let q =
          "SELECT o.*, c.name as customer_name FROM orders o INNER JOIN customers c ON o.customer_id=c.customer_id " +
          tsa +
          tso +
          " LIMIT ?, 10";
  ...
  addOrder = (req, res) => {
    try {
      ...
      new Promise((resolve, reject) => {
        let q =
          "INSERT INTO `orders`(`order_id`, `order_ref`, `customer_id`, `due_date`, `items`, `tax`, `grand_total`) VALUES (?,?,?,?,?,?,?)";
        db.query(
          q,
          [
            uniqid(),
