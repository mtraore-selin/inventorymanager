...
class Product {
      ...
      new Promise((resolve, reject) => {
        ...
        db.query(q, [req.body.start_value], (err, result) => {
          ...
          if (req.body.search_value != "") {
            return resolve({
              operation: "success",
              message: "search products got",
              info: { products: result, count: result.length },
            });
          }
          ...
          db.query(q, (err, result2) => {
            ...
            resolve({
              operation: "success",
              message: "10 products got",
              info: { products: result, count: result2[0].val },
            });
          });
        });
      })
  ...
  addProduct = (req, res) => {
    try {
      ...
      new Promise((resolve, reject) => {
        let q =
          "INSERT INTO `products`(`product_id`, `name`, `gender`, `size`, `material`, `category`, `description`, `product_stock`, `image`, `selling_price`, `purchase_price`) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        db.query(
          q,
          [
            uniqid(),
  ...
  updateProduct = (req, res) => {
    try {
      ...
      new Promise((resolve, reject) => {
        ...
        let q =
          "UPDATE `products` SET `name`=?,`gender`=?,`size`=?,`material`=?,`category`=?,`description`=?,`product_stock`=?," +
          ts +
          "`selling_price`=?,`purchase_price`=? WHERE product_id=?";
        db.query(
          q,
          [
            req.body.name,
        ...
        db.query(q, [req.body.product_id], (err, result) => {
          ...
          if (result[0].image != null) {
            p = new Promise((res, rej) => {
              let pathToFile =
                path.resolve("./") + "/public/uploads/" + result[0].image;
