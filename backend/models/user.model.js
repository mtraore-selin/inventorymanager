...
const { WRONG_MSG } = require("../constants");
...
class User {
    ...
    try {
      new Promise((resolve, reject) => {
        ...
        db.query(q, [req.body.email], (err, result) => {
          ...
          if (result.length == 1) {
            let d_password = CryptoJS.AES.decrypt(
              result[0].password,
              process.env.CRYPTOJS_SEED
            ).toString(CryptoJS.enc.Utf8);
            let t_password = CryptoJS.AES.decrypt(
              req.body.password,
              process.env.CRYPTOJS_SEED
            ).toString(CryptoJS.enc.Utf8);
            ...
            console.log("password email", result);
      ...
      console.log(error);
      res.send({ operation: "error", message: WRONG_MSG });
    }
  };
  ...
  getEmployees = (req, res) => {
    try {
      ...
      new Promise((resolve, reject) => {
        ...
        let q =
          "SELECT `user_id`, `user_name`, `address`, `email`, `permissions`, `timeStamp` FROM `user` WHERE user_role != 'admin' " +
          tsa +
          tso +
          " LIMIT ?, 10";
        db.query(q, [req.body.start_value], (err, result) => {
          ...
          if (req.body.search_value != "") {
            return resolve({
              operation: "success",
              message: "search employees got",
              info: { employees: result, count: result.length },
            });
          }
          ...
          db.query(q, (err, result2) => {
            ...
            resolve({
              operation: "success",
              message: "10 employees got",
              info: { employees: result, count: result2[0].val },
            });
          });
        });
      })
      ...
      new Promise((resolve, reject) => {
        ...
        db.query(q1, [req.body.email], (err1, result1) => {
          ...
          if (result1.length > 0) {
            ...
            let q2 =
              "SELECT `user_role_permissions`, `user_role_name` FROM `user_roles` WHERE `user_role_name` = 'employee'";
            db.query(q2, (err2, result2) => {
              ...
              let q3 =
                "INSERT INTO `user`(`user_id`, `user_name`, `address`, `email`, `password`, `permissions`, `user_role`) VALUES (?, ?, ?, ?, ?, ?, ?)";
              db.query(
                q3,
                [
                  uniqid(),
                  ...
                  req.body.email,
                  req.body.password,
                  result2[0].user_role_permissions,
                  result2[0].user_role_name,
                ],
                (err3, result3) => {
                  if (err3) {
                    return reject(err3);
      ...
      new Promise((resolve, reject) => {
        ...
        db.query(q1, [req.body.email], (err1, result1) => {
          ...
          if (result1.length > 0 && result1[0].user_id != req.body.user_id) {
            ...
            let q2 =
              "UPDATE `user` SET `user_name`=?,`address`=?,`email`=? WHERE `user_id`=?";
            db.query(
              q2,
              [
                req.body.name,
  ...
  getProfile = (req, res) => {
    try {
      ...
      new Promise((resolve, reject) => {
        let q =
          "SELECT `user_id`, `user_name`, `address`, `email`, `image` FROM `user` WHERE email = ?";
        db.query(q, [email], (err, result) => {
          ...
          resolve({
            operation: "success",
            message: "Employee profile got successfully",
            info: { profile: result },
          });
        });
      })
  ...
  updateProfile = (req, res) => {
    try {
      ...
      new Promise(async (resolve, reject) => {
          ...
          await new Promise((res, rej) => {
            ...
            db.query(qs, [email], (erra, resulta) => {
              ...
              if (resulta[0].image != null) {
                fs.unlink(
                  path.resolve("./") +
                    `\\public\\profile_images\\${resulta[0].image}`,
                  (errb) => {
                    if (errb) {
                      console.log(errb);
                      rej();
        ...
        let q =
          "UPDATE `user` SET `user_name`=?,`address`=?" +
          ts +
          " WHERE email = ?";
