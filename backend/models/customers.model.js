      ...
      new Promise((resolve, reject) => {
        ...
        db.query(q, [req.body.start_value], (err, result) => {
          ...
          if (req.body.search_value != "") {
            return resolve({
              operation: "success",
              message: "search customers got",
              info: { customers: result, count: result.length },
            });
          }
          ...
          db.query(q, (err, result2) => {
            ...
            resolve({
              operation: "success",
              message: "10 customers got",
              info: { customers: result, count: result2[0].val },
            });
          });
        });
      })
    ...
    try {
      new Promise((resolve, reject) => {
        ...
        db.query(q1, [req.body.email], (err1, result1) => {
          ...
          if (
            result1.length > 0 &&
            result1[0].customer_id != req.body.customer_id
          ) {
            resolve({
              operation: "error",
              message: "Duplicate customer email",
            ...
            let q2 =
              "UPDATE `customers` SET `name`=?,`address`=?,`email`=? WHERE `customer_id`=?";
            db.query(
              q2,
              [
                req.body.name,
  ...
  deleteCustomer = (req, res) => {
    try {
      new Promise((resolve, reject) => {
        ...
        db.query(q, [req.body.customer_id], (err) => {
          if (err) {
            return reject(err);
          }
          resolve({
